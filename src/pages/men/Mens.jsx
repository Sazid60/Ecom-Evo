...
const Mens = () => {
    const { data: allProducts, isLoading } = useQuery({ queryKey: ['menCategory'], queryFn: fetchProducts });
    const [currentPage, setCurrentPage] = useState(1);
    const [isSidebarOpen, setIsSidebarOpen] = useState(false); // State to manage sidebar visibility
    const productsPerPage = 10;

    const menProducts = Array.isArray(allProducts)
        ? allProducts.filter(product => product?.category === 'Men')
        : [];
    ...
    return (
        <>
            <SecondaryTitle title={"For Him"} titleDesc={"Explore the vast collection for men here"} />
            <MainContainer>
